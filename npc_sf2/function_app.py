...
# Config
# -----------------------------------------------------------------------------
SOURCE_GRPC_ADDRESS = os.getenv("SOURCE_GRPC_ADDRESS", "localhost:50052")
DEST_GRPC_ADDRESS = os.getenv("DEST_GRPC_ADDRESS", "localhost:50051")

SOURCE_TOPIC = os.getenv("SOURCE_TOPIC", "ContactChangeEvent")
DEST_TOPIC = os.getenv("DEST_TOPIC", "AccountChangeEvent")
...
def save_to_nsps_sf1(data: dict, source_system: str) -> None:
    """
    Save event payload into NSPS_SF1 database based on change type.
    """
    ...
    if change_type == "DELETE":
        delete_account_sf1(account_id)
        logger.info("Deleted from NSPS_SF1 database: %s", account_id)
        ...
        logger.info("Updated NSPS_SF1 database for %s", account_id)
        ...
        logger.info("Inserted into NSPS_SF1 database: %s", account_id)


def process_single_event(event) -> None:
    """
    Process one event from Server2, forward to Server1, then save to NSPS_SF1 DB.
    """
    ...
    if source == "Server1":
        logger.info("Skipping event because it already originated from Server1")
        return

    # Preserve origin as Server2 before forwarding
    ...
    with grpc.insecure_channel(DEST_GRPC_ADDRESS) as channel_dest:
        ...
        logger.info("Forwarded to Server1, publish status=%s", resp.status)

    save_to_nsps_sf1(data, source_system="NPC_SF2")


def bridge_server2_to_server1_once(max_run_seconds: int) -> int:
    """
    ...
    logger.info(
        "Bridge Server2 -> Server1: connecting. Source=%s Dest=%s (will run until stream end/error or %ss cap)",
        SOURCE_GRPC_ADDRESS,
        DEST_GRPC_ADDRESS,
        max_run_seconds,
    )
...
def timer_trigger(myTimer: func.TimerRequest) -> None:
    ...
    processed = bridge_server2_to_server1_once(MAX_RUN_SECONDS)
