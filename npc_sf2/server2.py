...
Server2 - listens on 127.0.0.1:50052.
"""
import grpc
...
class PubSubService(pubsub_pb2_grpc.PubSubServicer):
    ...
    def Publish(self, request, context):
        ...
        print(f"Event Published (Server2) -> {request.topic_name}")
        return pubsub_pb2.PublishResponse(status="SUCCESS")

    def Subscribe(self, request, context):
        topic = request.topic_name
        replay = request.replay_preset
        print("Subscriber connected to Server2")
        ...
        while True:
            if not context.is_active():
                print("Subscriber disconnected from Server2")
                break
            with LOCK:
                ...
                while last_index < len(events):
                    ...
                    last_index += 1
                    print(f"Sending Event (Server2) -> {event.replay_id}")
                    yield event
            time.sleep(0.5)
